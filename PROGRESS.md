...
| ~~`GEMINI_API_KEY` not yet obtained~~ | **RESOLVED.** Added by the owner (2026-08-13) to `backend/.env` — classification/extraction now run for real instead of degrading to null, once the backend process picks it up (needs a restart if it was already running) | Owner ✅ |
| ~~Tesseract not installed on the owner's native (non-Docker) machine~~ | **RESOLVED.** Installed (2026-08-13, 5.5.3) — all 6 previously CI-only, Tesseract-gated tests now also pass locally; **154/154 backend tests, 0 skipped** | Owner ✅ |
